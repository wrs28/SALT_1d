...
function wavePlot(ψ::Union{Array{Complex{Float64},1}, Array{Complex{Float64},2}},
    ...
    if array
        ...
        pcolormesh(x₁, x₂, transpose(real(sqrt.(ɛ_sm+inputs.D₀*F_sm)))) #transpose(r))
        xlim( [ ∂R[1],∂R[2] ] )
        ylim( [ ∂R[3],∂R[4] ] )
        axis("tight")
        xlabel("Real")
        ...
        pcolormesh(x₁, x₂, transpose(imag(sqrt.(ɛ_sm+inputs.D₀*F_sm))), cmap="bwr")
        xlim( [ ∂R[1],∂R[2] ] )
        ylim( [ ∂R[3],∂R[4] ] )
        clim([-1,1]*findmax(abs.(imag(sqrt.(ɛ_sm+inputs.D₀*F_sm))))[1])
        axis("tight")
        xlabel("Imag")
